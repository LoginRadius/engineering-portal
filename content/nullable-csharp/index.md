...
As described above, The Nullable types used to assign the null value to the value data type. That means we can assign a null value directly to a variable that is of the value data type. We can declare null value by using `Nullable<T>` where `T` is a type like an int, float, bool, etc.
...
There are 2 ways to declare Nullable types.
...
Nullable types have 2 properties.
1. HasValue
2. Value
...
**Value**: This property gives the value of the Nullable type variable. If the variable has some value then it will return the value else it will give the runtime `InvalidOperationException` exception when the variable value is null.
...
To use the Nullable type as a local variable it should be declared first else it will give a compile-time error. This rule is similar to the value data type.
...
We can not assign the Nullable type variable value to the non-nullable type variable directly. Like in the example below if we try to assign the value then it will give the compile-time error
...
For the Nullable variable, we can use the null coalescing operator (??) to check if the variable value is `null` or not, and then we can assign the non-nullable type variable value according to that. This operator can be used when we are unsure that at run time if that Nullable variable's value is changed according to our logic or not. Here is an example of that
...
```
In the above example as we can see that if the variable a value is null then it will assign the value 10 to b variable. In that case, b is assigned with value 10 and the printed value will be 10.
 
## Conclusion

In this article, We have discussed the Nullable types and it's properties and methods as well. The main advantage of using the Nullable types is that we can store the Null value in a column of a database using this type.