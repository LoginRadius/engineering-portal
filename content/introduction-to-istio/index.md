...
Istio is Open Source service mesh (developed in partnership between teams from Google, IBM, and Lyft) which provides a dedicated infrastructure layer for creating service-to-service communication safe, fast, and reliable. Having such a fanatical communication layer can provide variety of advantages , like providing observability into communications, providing secure connections, or automating retries and backoff for failed requests.
...
Istio does this by adding a sidecar proxy which intercepts all network communication between microservices, then configure and manage Istio using its control plane functionality which incorporates,
 1. Granular control over the service-service communication and its routing with additional functionality of retries, fault injection, circuit breakers.
 2. Providing secure mTLS without any changes in application code.
...
The data plane is composed of Envoy proxy deployed as sidecars. Envoy itself is a L7 proxy and communication bus designed for modern microservices based architecture. These proxies intercepts and control all network communication between microservices. They also collect and report telemetry on all mesh traffic.
...
Responsible for controlling the authentication and identity management between services. Allow developers to buld a zero-trust network based on service identity.
...
It is the basic features of istio which facilitate the routing between services. Istio simplifies the configuration of service-level properties like circuit breakers, timeouts, and retries.
...
For discovering all the services in the ecosystem, Istio connects to the Service discovery System and populates its own service registry. This registry is then used by the Envoy sidecar proxy to route traffic to the correct service.
...
It is used to manage inbound and outbound traffic for your mesh, letting you specify which traffic you want to enter or leave the mesh. Gateway configurations are applied to standalone Envoy proxies that are running at the edge of the mesh, rather than sidecar Envoy proxies running alongside your service workloads. Using this, we can expose our services to the internet.  
...
**istio: ingressgateway** is the gateway which is enabled by default after installation. We can create our own custom gateway. Here, the hosts **my-svc.example.com** will resolve to the load balancer provided by the istio by default. To use this gateway, one has to add config in the virtual service like for example:
...
### Service Entries
This object is used to add an external service as part of the service mesh which can include a service running in a VM or other K8s cluster in case of multi-cluster installation.

A typical example would be connecting a service to database cluster which is not part of mesh.
```yaml
apiVersion: networking.istio.io/v1alpha3
kind: ServiceEntry
metadata:
...
spec:
  ...
  resolution: DNS
```
The Service Entry should be in the same namespace as that of calling service. This is helpful especially in the case where the service is not exposed to a public endpoint and can be accessed using internal service DNS like above example.
...
spec:
  ...
  mtls:
    mode: STRICT
```
Here we are defining a peer authentication object for a service labeled **my-svc** which tells that any service that needs to talk to **my-svc** will communicate using mtls and the service will accept only TLS connection. By default, istio enabled **PERMISSIVE** mode which means accepts both plaintext and encrypted communication. 

We can define peer authentication on the mesh, namespace, and pod level.

This is all for the introduction to istio. In next part, we will take a look at installing Istio and configuring services to use Istio.
