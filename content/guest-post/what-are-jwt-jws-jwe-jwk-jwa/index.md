...
### Structure
JWT is mainly composed of three parts: the header, payloadm and signature that are Base64 URL-encoded. 
...
### Structure
The JWE compact serialization form has 5 main components: 
...
- The JOSE Header being the first element of the token is same as the headers of the previously mentioned JWT and JWS. 
...
- Some encryption algorithms require an initialization vector, which is a randomly generated number that is used along with a secret key to encrypt data. This prevents repeated encryption of the same data using the same secret key. The recepient requires this initialization vector to decrypt the message, and hence, is placed in the JWE token.
...
- The JWE Authentication tag is the last part of the JWE Token that is generated along with the ciphertext. It ensures the integreity of the ciphertext.
