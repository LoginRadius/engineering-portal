...
### Step 3: Set Up the API Client Class
...
The `LoginScreen` UI code is similar to the `RegistrationScreen` in that it also has two `TextFormField` widgets that serve as your `email` and `password` fields, as well as an `ElevatedButton` to handle event submission.

So, for the sake of brevity, I’ll be leaving out the `LoginScreen` UI code and focusing mainly on the code to be written in the `ElevatedButton` onPressed callback.
...
To display the result of your `getUserData` method on your screen, you use a `FutureBuilder` widget. The code snippet below shows how you can use the `FutureBuilder` to get the results of the `getUserData` future.
...
LoginRadius provides high-level, secure, and well-documented APIs to simplify the implementation of authentication and user identity management. This tutorial discussed what LoginRadius is, its benefits, and how to use the LoginRadius APIs to handle user authentication and registration in a Flutter application.

The source code of the demo application is available on [GitHub](https://github.com/iloveteajay/loginradius_example).
