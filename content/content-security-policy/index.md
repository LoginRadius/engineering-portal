...
### Overview
A Content Protection Policy (CSP) is a security standard that adds an extra layer of defense in the detection and mitigation of certain kinds of attacks, such as Cross-Site Scripting (XSS), clickjacking, and other code injection threats. CSP is a preventative step against attacks that rely on executing malicious material in a trusted web context, as well as other attempts to bypass the same-origin policy.

### What Threats CSP Can Mitigate?

#### 1. Mitigating cross-site scripting

The CSP's main purpose is to prevent and report XSS attacks. XSS attacks take advantage of the browser's faith in the server's content. Because the browser trusts the source of the material so without additional safety measures, the browser runs all code from a trustworthy origin and is unable to distinguish which code is legal, thus any injected malicious code is also executed.

The website administrator can reduce the XSS attack using the CSP, by defining trusted source sites for the executable scripts, When we will use CSP header browsers only allows us to run the script from the whitelisted domains, and ignore all other scripts.

We can also use same-origin policy (SOP) header for preventing the website from accessing data from the other origin but Websites need to include lots of assets from external sources like content delivery networks (CDNs), Google Analytics scripts, fonts, styles, comment modules, social media buttons, etc. so for the modern web we need to use CSP  


#### 2. Mitigating Packet Sniffing and Enforcing HTTPS
One interesting advantage of a content security policy is we can define the permitted protocols For example, the sites can restrict browsers to load content over HTTPS, some browsers by default will not connect to HTTPS but using the content security policy we can enforce browsers to encrypt conversations with your server. 
...
#### User case #2: Trused Domain only
...
Suppose you are running a ecommerce site and wants to ensure that all resources are only loaded via SSL or HTTPS.The below policy make ensure that all of the resources on the your website load from TLS
...
If you want to allow users of a web application to add images/photos from any source, but permits all scripts from trusted sources or specfic source.
...
```javascript
 ...
 If you want to add social widgets like google+ button, Facebook like,Tweet button on your website, need to allow external script also like below policy
...
```javascript
 ...
 ```
In the above example policy only allows the script from cdn.loginradius.com.
...
<body>
 ...
 ```
In the below sample HTML browser trying to download javascript from the other source but we have allowed javascript src only from the CDN so the browser will send the following violation report
...
The Web Application Security Working Group of the Wide Web Consortium (w3c has already began work on the specification's next [version, Content Security Policy Level 3]("https://www.w3.org/TR/CSP3/") and Content Security Policy Level 2 already [Candidate Recommendation]("https://www.w3.org/TR/CSP2/").
